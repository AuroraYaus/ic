...
| # | 文件名 | 核心内容 |
|:---|:---|:---|
| 24 | `Makefile常见错误50例.md` | 50 个高频错误，分类逐一剖析（错误现象→根因→修复→预防）：**第一类：语法/格式错误（10 例）** — (1) 配方用空格而非 TAB (2) 编辑器 TAB→空格转换 (3) 条件指令缩进破坏解析 (4) 续行符 `\` 后有空格/制表符 (5) 变量引用漏括号 `$VAR` 而非 `$(VAR)` (6) 变量名含空格或特殊字符 (7) `define` 块内 TAB 混乱 (8) 配方空行用空格/TAB 而非完全空行 (9) 文件末尾缺少换行导致最后一行被忽略 (10) 注释 `#` 出现在配方行中按字面执行。**第二类：变量展开陷阱（12 例）** — (11) `=` 递归展开导致死循环 (12) `$(shell ...)` 用 `=` 每次引用都重新执行 (13) `+=` 在简单展开变量上追加时丢失原有值 (14) 自动变量 `$@` 在变量定义中为空——只在配方中有效 (15) 变量名拼写错误被静默展开为空 (16) `export` 传递了不该传递的变量 (17) 命令行 `make VAR=val` 覆盖与 `override` 的优先级混淆 (18) 条件判断中引用尚未定义的变量 (19) `$(origin ...)` 返回值的误解（`environment` vs `environment override`）(20) `$$` 转义层数错误：`eval` 内需要 `$$$$` (21) Target-specific 变量的作用域意外波及所有前置条件 (22) 单字母自动变量遗留历史值。**第三类：依赖/目标错误（10 例）** — (23) 目标与目录同名：`make build` 看到已有 `build/` 目录跳过 (24) 忘记 `.PHONY` 声明 (25) Order-Only 前置条件用空格 ` ` 而非 `|` (26) 模式规则 `%` 匹配了不该匹配的文件 (27) 隐含规则悄悄地编译了不该编译的文件 (28) `VPATH` 找到了错误的同名文件（"影子源文件"）(29) 循环依赖——A 依赖 B，B 依赖 A (30) 依赖列表中混杂了不存在的文件（make 尝试构建不存在的 `.h`）(31) 自动依赖 `.d` 文件生成规则和目标在同一个配方中失败 (32) `include` 的文件被 remake 后 Make 状态不一致。**第四类：并行构建陷阱（8 例）** — (33) 两个独立目标竞争写入同一临时文件 (34) `mkdir -p` 与文件创建的竞态 (35) 静态库并行构建时 `ar` 的索引损坏 (36) `.NOTPARALLEL` 误用导致构建时间暴增 (37) `-j` 下配方输出交错乱码（未用 `--output-sync`）(38) 并行时 `clean` 目标的破坏性删除与正在进行的编译竞争 (39) `$(shell ...)` 在并行环境下的随机执行顺序 (40) `make -j` 传递失败——子 Make 用了 `make` 而非 `$(MAKE)`。**第五类：跨平台/可移植性（5 例）** — (41) 配方用 Linux 命令（如 `sed -i`）在 macOS 上行为不同 (42) 路径分隔符 `/` vs `\` (43) `echo` 的 `-n`/`-e` 行为差异 (44) Shell 差异：`/bin/sh` 指向 dash vs bash (45) 文件名大小写在 macOS（不敏感）vs Linux（敏感）上的差异。**第六类：性能反模式（5 例）** — (46) `$(shell find ...)` + `$(foreach ...)` 每次 make 都扫描 (47) `clean` 目标无依赖无 `.PHONY` 每次运行代价高 (48) `-j` 设置过大导致 I/O 颠簸 (49) `$(eval ...)` 在配方内调用（每次执行都重新解析 Makefile）(50) 不必要的递归 Make 导致的序列化瓶颈 |
