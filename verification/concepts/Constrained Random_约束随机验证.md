...
- [[verification/concepts/Coverage Model_覆盖率模型|覆盖率模型（Coverage Model）]] — Covergroup 定义 "验证什么"，约束随机引擎定义 "激励空间"，两者通过覆盖率缺口反馈驱动约束迭代收敛，构成 CDV 的闭环核心
- [[verification/concepts/UVM Methodology_UVM方法学|UVM 验证方法学]] — UVM Sequence 机制与 rand/constraint 深度集成，Sequence Item 是 CRV 的数据载体，Sequencer-Driver 流水线是 CRV 激励注入的执行通道
- [[verification/concepts/Testbench Architecture_验证平台架构|验证平台架构（Testbench Architecture）]] — Agent 的 Sequencer 是 CRV 的执行入口，Scoreboard 和 Monitor 是激励效果的检查点——激励的随机性必须被观测和检查环完整闭环
- [[verification/concepts/SVA_SystemVerilog断言|SystemVerilog 断言（SVA）]] — CRV 提供激励多样性的宽度，SVA 在每个激励下提供时序正确性的深度检查，两者覆盖验证的互补维度
