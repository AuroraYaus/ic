...
- **时序收敛是横跨全流程的迭代闭环**：每步都需 STA 验证，Setup 违例需回到前序阶段修复
- **MCMM 多工艺角多模式同时满足时序**：在 SS/FF/TT/SF/FS 等多个 PVT 角和 Func/Test/Sleep 等多个模式下同时满足时序
- **OCV 到 LVF 从保守降额到统计建模**：反映了方法论从保守降额到统计建模的转变，减少了过度设计
- **串扰延迟变化需基于时序窗口分析**：同一路径在不同邻居活动组合下的延迟不同，需要基于耦合电容和开关窗口（Timing Window）的 STA 分析
- **IR-drop 是最隐蔽的时序违例来源**：导致局部 VDD 降低、单元延迟增加，可能在 Signoff 后才发现新违例
- **Useful Skew 是 Setup 收敛的利器**：通过主动制造时钟偏移来平衡相邻寄存器级的延迟差
- **Setup 加速数据路径，Hold 延迟数据路径**：Setup 侧重加速（低 Vth、减小负载），Hold 侧重延迟（插入缓冲器/延迟单元）
- **ECO 是时序收敛后期的主要修复手段**：强调局部修改、最小化对已完成布线的影响
- **PPA 三角权衡是时序收敛的核心矛盾**：加速路径意味着更大的单元面积和更高的漏电流

## 面试高频问答

### Q10: 什么是时序收敛（Timing Closure）？常见优化手段有哪些？

时序收敛（Timing Closure）是指**在数字IC物理实现流程中，通过多轮迭代使芯片内所有时序路径在所有工艺角（Corner）和工作模式（Mode）下均满足建立时间（Setup）和保持时间（Hold）约束的完整工程过程**。时序收敛不是一次性任务，而是跨越逻辑综合（Synthesis）、布局（Placement）、时钟树综合（Clock Tree Synthesis, CTS）和布线（Routing）的闭环迭代——每个阶段输出延迟数据，STA 分析违例，根据违例类型和严重程度决定是进入下一阶段还是回到前一级修复。时序收敛完成（所有 Slack >= 0、所有 DRC 清零）是 Tape-Out 的必要前置条件。

**常见优化手段按阶段分类**：

**（一）综合阶段优化**：(1) 时序驱动的逻辑综合——SDC 约束引导综合引擎在技术映射（Technology Mapping）时选择速度更优的单元架构；(2) 寄存器重定时（Retiming）——在组合逻辑中移动寄存器位置以平衡各级延迟；(3) 资源共享和逻辑复制——在关键路径上复制逻辑减少扇出和线长。

**（二）布局阶段优化**：(1) 关键路径感知的布局——STA 反馈驱动布局引擎将关键路径上的寄存器放得更近；(2) 宏单元（Macro）规划——大型 SRAM/IP 的位置对关键路径影响极大——摆放不当会在宏单元之间制造无法走线的"峡谷"；(3) 拥塞（Congestion）缓解——布线拥塞区域导致绕行（Detour），间接增长关键路径延迟。

**（三）CTS 阶段优化**：(1) 有用偏斜（Useful Skew）——有意延迟捕获时钟来"借"时间给 Setup 路径，是 Setup 收敛的关键武器；(2) 公共路径比例优化——增大 Launch 和 Capture 时钟的公共路径部分（Common Path Ratio），减少 OCV 降额的悲观量；(3) 时钟门控（ICG）位置优化——ICG 靠近寄存器组减少局部时钟延迟。

**（四）布线阶段及后布线优化（Post-Route ECO）**：(1) 单元尺寸调整（Cell Sizing）——关键路径 Upsize（增大驱动）或替换为 LVT/SLVT 单元；(2) 缓冲器插入（Buffer Insertion）——修复 Max Transition/Max Capacitance 和 Hold 违例的主力手段；(3) 延迟单元插入（Delay Cell Insertion）——针对性修复 Hold 违例；(4) 重新布线（Re-Routing）——对关键线网使用更高金属层（更低的单位长度电阻）或更宽金属线；(5) 电压阈值混合（Multi-Vth Optimization）——关键路径用 LVT（快但漏电大）、非关键路径用 HVT（慢但漏电小），实现 PPA 最优平衡。

**（五）约束与裕量优化**：(1) 时钟不确定性缩减——从综合阶段的大裕量（200-300ps）逐步递减到 Signoff 阶段的最小裕量（20-50ps）；(2) OCV 方法论升级——从 Flat OCV → AOCV → POCV/LVF 逐步降低悲观性，回收数十 ps 的时序预算；(3) CRPR（时钟路径悲观去除）——回收公共时钟路径上的双重降额，典型回收 20-50ps；(4) 异常路径约束审计——确认所有多周期路径和伪路径约束正确，防止工具对真实时序路径漏检或对不可能路径做无效优化。

**时序收敛的整体策略是"由粗到精"**：综合阶段使用理想时钟和粗略的 WLM 延迟模型快速收敛到"大致正确"；物理设计各阶段逐步用更精确的延迟模型替代估算模型——布局后引入真实单元位置，CTS 后引入真实时钟延迟，布线后引入真实 RC 寄生参数——每一步精确度的提升伴随着新违例的暴露和修复。Slack 从综合阶段的"深负"逐步改善到 Signoff 阶段的"全正"，是时序收敛成功的直观标志。

**关键要点：**
- **时序收敛是跨全流程的闭环迭代**：综合 → 布局 → CTS → 布线 → STA → ECO——每一个环节都影响下一个环节的时序起点
- **Useful Skew 是 Setup 收敛的利器**：有意延迟捕获时钟以"借"时间——零面积开销、纯时钟调整，是最优雅的 Setup 修复手段
- **Multi-Vth 实现 PPA 最优平衡**：关键路径用 LVT/SLVT（快、漏电大）、非关键路径用 HVT（慢、漏电小）——以可控的漏电增加换频率提升
- **OCV 悲观性的逐步回收是收敛关键**：Flat OCV → AOCV → POCV → LVF + CRPR，累计回收可达 50-150ps——对 GHz 级设计是生死攸关的余量
- **约束质量决定收敛效率**：错误的多周期/伪路径约束误导优化方向——约束审计是时序收敛中最容易被忽视但影响最大的环节
- **时序收敛的度量标准是 Slack ≥ 0 + DRC 清零**：所有 MCMM 场景、所有路径 Slack 非负、所有 Max Transition/Max Capacitance 违例清零——这是 Signoff 的硬性门槛
- **修复顺序：先 Hold 后 Setup、先 DRC 后时序**：Hold 在 FF 角最差、修复影响范围小（插入 buffer）；DRC 是时序计算可信的前提——违例状态下 STA 报告不可信

## 与其他概念的关系

- [[asic-flow/concepts/逻辑综合|逻辑综合（Synthesis）]] — 综合阶段的 WLM 时序估算决定网表质量，好的起始点大幅降低物理阶段的时序收敛难度
- [[asic-flow/concepts/布局布线|布局布线（P&R）]] — 宏单元和标准单元的物理位置决定了关键路径的线长和延迟上限
- [[asic-flow/concepts/时钟树综合|时钟树综合（CTS）]] — CTS 直接决定时钟偏斜和不确定性，是 Setup/Hold 违例分析的核心变量
- [[asic-flow/concepts/签核|物理签核（Signoff）]] — Timing Signoff 是 Tape-Out 前的最终 STA 检查，包括 Setup、Hold、DRV、Noise 等多维度验证
- [[cross-domain/concepts/低功耗设计|低功耗设计]] — 多电压域设计引入 Level Shifter 和多个电压轨，增加了 MCMM 场景数量和时序收敛复杂度
- [[concepts/半导体基础|半导体基础]] — PVT 变异的物理根源：掺杂浓度偏差、氧化层厚度波动、温度对载流子迁移率的影响
