...
> **本文从方法论与 Signoff 视角**组织：侧重 CDC 验证策略、SDC 时序约束（false_path/max_delay）、形式验证方法、时钟域划分与 CDC 路径处理策略、准静态信号豁免流程。对于 RTL 实现视角（可综合编码模板、电路结构、综合属性标注），参见 [[rtl-design/concepts/跨时钟域设计|跨时钟域设计（RTL 实现视角）]]。

## 原理

### 亚稳态的根源与 MTBF

亚稳态的完整物理机制和 MTBF 公式的数学推导见 [[concepts/亚稳态|亚稳态（Metastability）]]。在 CDC 方法论层面，MTBF 被用作同步器级数决策和可靠性验证的量化依据——目标值通常为 1000 年（约 $3.15 \times 10^{10}$ 秒）以确保量产芯片的鲁棒性。解析时间常数 $\tau$ 与工艺强相关（40nm 约 20-50ps，7nm 约 5-10ps），工艺越先进同步器所需级数越少。单片 SoC 中数百条独立 CDC 路径的累计 FIT 预算要求在系统级可靠性目标下为每条路径设定比直观判断高 2-3 个数量级的 MTBF 目标。
