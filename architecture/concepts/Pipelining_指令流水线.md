...
- [[architecture/concepts/Out-of-Order_乱序执行|乱序执行（Out-of-Order Execution）]] — 乱序执行在流水线基础上通过动态调度进一步提升 ILP，Tomasulo 算法本质上是对流水线 RAW 冒险的动态化解
- [[architecture/concepts/Branch Prediction_分支预测|分支预测（Branch Prediction）]] — 分支预测是流水线控制冒险的高级解决方案，预测精度直接影响流水线停滞频率
- [[architecture/concepts/Memory Hierarchy_存储层次|存储层次（Memory Hierarchy）]] — Load-Use Hazard 的阻塞周期与缓存命中时间直接相关，缓存缺失会成倍延长 Load 的 MEM 阶段并级联阻塞后续所有依赖指令
- [[concepts/CMOS Fundamentals_CMOS基础|CMOS 基础]] — 时钟频率和流水线级数的物理极限由 CMOS 工艺的晶体管开关速度和连线延迟决定，流水线寄存器自身的 Clock-to-Q + Setup 开销设定了增加级数的递减回报点
