...
- **内存墙缓解双管齐下**：需"增加带宽 + 减少数据移动"——近存计算和 HBM 是两端关键技术
- **IOMMU/SMMU 实现 I/O 一致性**：消除 DMA 缓冲区和 CPU 缓存间显式同步开销
- **三级功耗管理框架**：DVFS + 时钟门控 + 电源门控构成，PMU 协调状态转换序列
- **Chiplet 突破掩模尺寸限制**：通过 UCIe/BoW 使多 Die 封装表现为单一 SoC，掩模尺寸不再是单芯片上限
- **嵌入式 SoC 安全三件套**：TrustZone + Secure Boot + 内存加密；车规额外需 ASIL 功能安全
- **CXL 桥接 PCIe 和一致性总线**：在 I/O 物理层上实现 CPU-Cache-Coherent 协议
- **Flynn 分类法与 Amdahl 定律**：前者指导 SoC 中 CPU、GPU、NPU 的组合选择；后者解释了大核 + 小核异构策略的必然性
- **复杂 SoC 的电压与时钟域**：通常包含 10-30 个独立电压域和 20-50 个不同频率的时钟域，跨域通信的 CDC 同步器面积可达总逻辑面积的 3-5%
- **Chiplet Die-to-Die 带宽**：受限于 PHY 传输速率——UCIe 标准封装模式每链路 8-16 GT/s，先进封装模式（硅中介层）可达 24-32 GT/s
- **SoC 设计 NRE 成本**：一次性工程费用在 5nm 节点可达 2-4 亿美元，Chiplet 复用可摊薄后续产品的 NRE
- **SLC 分配策略至关重要**：系统级缓存的容量分配给 CPU、GPU 和 NPU 的独占/共享比例直接影响各子体性能
- **中断控制器拓扑集成**：GICv3/v4 涉及 ITS（Interrupt Translation Service）、MSI 和亲和性路由，是异构多核系统的核心调度基础设施
- **SoC 验证复杂性超线性增长**：连接性检查、协议检查、性能验证和死锁检查在 100+ IP 的 SoC 中需要数月的验证周期
...
- [[architecture/concepts/片上总线|片上总线（On-Chip Bus）]] — 互连拓扑和协议选择是 SoC 架构的核心决策，AXI/CHI/NoC 各适用于不同规模系统
- [[architecture/concepts/缓存一致性|缓存一致性（Cache Coherence）]] — CXL 和 CHI 的一致性模式将 I/O 设备和加速器纳入与 CPU 相同的一致性域
- [[architecture/concepts/存储层次|存储层次（Memory Hierarchy）]] — SoC 片内 SRAM、多级缓存到片外 DRAM/HBM 的层次划分是 PPA 优化的核心
- [[cross-domain/concepts/低功耗设计|低功耗设计（Low-Power Design）]] — 电压域划分、DVFS、电源门控和时钟门控是 SoC 功耗管理的核心技术，PMU 协调各域的上/下电序列并维护状态一致性
- [[cross-domain/concepts/跨时钟域设计|跨时钟域（CDC）]] — SoC 中数十个异步时钟域的跨域通信依赖异步 FIFO 和 CDC 同步器，域划分策略直接影响芯片面积和时序收敛难度
- [[architecture/concepts/分支预测|分支预测（Branch Prediction）]] — CPU 核心前端的分支预测器决定取指带宽的连续性，在多核 SoC 中各核心独立预测，但共享的 I-Cache 和 BTB 的组织影响取指延迟
- [[cross-domain/concepts/复位策略|复位策略（Reset Strategy）]] — SoC 上电序列需协调多电源域、多时钟域的复位释放顺序，PMU 的安全状态机确保各 IP 在复位释放前到达已知安全状态
