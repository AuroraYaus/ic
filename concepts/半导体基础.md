...
**超频视角：温度越低为什么超频越高**——超频（Overclocking）的本质是提高时钟频率与电源电压以缩短关键路径延迟、提升最高工作频率 Fmax（Maximum Frequency）。超频者通常同时拉高 $V_{DD}$（如 1.3-1.5V），使工作点远离阈值电压、处于高过驱动区——此时 $I_{on}$ 的温度竞争由迁移率主导（$V_{th}$ 随温度的漂移相对大过驱动量 $V_{GS}-V_{th}$ 占比很小）：温度越低，晶格散射越弱、迁移率越高，门延迟越短，Fmax 越高。降温的附加收益还包括：漏电随温度指数下降（功耗与热预算余量）、金属互连电阻随温度线性下降（RC 延迟减小）、电迁移与老化速率减缓（可靠性余量）。极限超频（液氮 -196°C）正是把迁移率主导区的低温收益推到极致——但商业芯片规格工作范围之外的低温超频未经厂商验证，还可能遇到 PLL 锁定、DRAM 刷新等系统级问题，属工程风险自担。反过来，若在近阈值低 $V_{DD}$ 区"降频省电"场景，降温反而可能变慢（温度反转），降温策略必须先确认工作点。
...
- **低温提速只在迁移率主导区成立**：高 VDD 超频场景越冷越快；近阈值低 VDD 场景发生温度反转——高温反而快——降温策略必须先确认工作点
