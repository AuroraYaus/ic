...
**扫描触发器（Scan Flip-Flop / Mux-D Flip-Flop）** 是 DFT（可测试性设计）的基础单元。标准 D-FF 在数据输入端增加一个 2:1 MUX，由扫描使能（Scan Enable, SE）信号选择功能数据（D）或扫描输入（Scan-In, SI）。在测试模式下，将芯片中所有扫描触发器串联为一个或多个**扫描链（Scan Chain）**——扫描数据从测试仪通过 SI 端口串行移入所有寄存器、执行一个功能时钟周期捕获电路响应、再通过 SO 端口串行移出观测。扫描链上的每个 Mux-D FF 额外增加约 5-10% 的面积开销（MUX 的传输门和反相器）和约 5% 的 D 端建立时间增加（MUX 的传播延迟）。现代 SoC 使用压缩扫描（Scan Compression）技术——在扫描链中插入解压缩器/压缩器（Decompressor/Compactor）将数百条内部短扫描链映射到少量测试仪通道，将测试数据量和测试时间降低一个数量级。

### 保持寄存器与功耗门控

**保持寄存器（Retention Register / Retention Flip-Flop）** 是功耗门控（Power Gating）的关键元件。当芯片进入休眠模式时，主电源域（VDD）被关断，但保持寄存器由独立的常通电源域（VDD_RET）供电，保存关键状态（如配置寄存器、程序计数器）。保持寄存器在结构上是标准 D-FF 加一个"影子锁存器"（Balloon Latch）——在掉电前将主寄存器的状态保存到影子锁存器（由 VDD_RET 维持），恢复上电后将影子锁存器的值写回主寄存器。保持寄存器的面积约为标准 D-FF 的 2 倍，保持功耗是正常静态漏电功耗的 1-5%。保持寄存器的保存/恢复控制信号（Save/Restore）需要由常通域的功耗管理单元（PMU）产生，且其时序不依赖于被关断的时钟。

### 脉冲锁存器

**脉冲锁存器（Pulsed Latch）** 是高性能处理器中替代传统主从 D-FF 的备选元件。脉冲锁存器不包含完整的主从结构——它由一个电平敏感锁存器和一个脉冲生成器（Pulse Generator）组成，脉冲生成器将时钟沿转换为一个持续时间约 100-200ps 的窄脉冲，锁存器在此脉冲窗口内透明（Transparent）采样数据。脉冲锁存器面积约为 D-FF 的 65%，时钟功耗因时钟负载电容减少约 40-50%。但脉冲锁存器对时钟占空比和脉冲宽度极其敏感——PVT 变化可能导致脉冲宽度不足（建立时间违反）或过大（保持时间违反/穿透竞争），且脉冲锁存器的时序建模比边沿触发器更复杂。



### 多比特寄存器与时钟偏斜

在 64 位或更宽的数据总线中，寄存器组（Register Bank）由 64 个并行的 D-FF 组成，共享同一时钟信号。实际物理实现中，时钟树从时钟源到各个寄存器的延迟存在**时钟偏斜（Clock Skew）**——时钟到达不同比特的到达时间差异可因局部时钟树布线差异达到 5-15ps。当数据总线中的不同比特需要同时在寄存器间传输时，时钟偏斜在约束中体现为：源寄存器和目的寄存器之间的偏斜差（Local Skew）加入或减去建立/保持裕量。STA 使用 **CRPR（Clock Reconvergence Pessimism Removal）** 补偿共享的公共时钟路径部分——源寄存器和目的寄存器共享时钟树的上游部分（偏斜被抵消），仅考虑从公共节点分叉到两个寄存器的路径段。在布局布线策略中，关键数据总线寄存器组通常使用同一局部时钟缓冲器（Local Clock Buffer）驱动以保证最小的局部偏斜。
...
- 数据通路中的 Pipeline（流水线寄存器）本质上是对组合逻辑路径的时序切片——时序逻辑 + 组合逻辑 + 时序逻辑的交替模式是同步设计的标准单元
- 异步复位仿真模型需要额外的仿真脚本或工具（如 VCS 的 +neg_tchk 或 NCSIM 的 tcheck）来检测恢复/移除时间违反，因为标准 Verilog 仿真引擎在零延迟模型下不自动检查这些异步时序约束
- 先进工艺中（<7nm），偏置温度不稳定性（BTI, Bias Temperature Instability）和 Hot Carrier Injection（HCI）等老化效应导致 D-FF 的建立/保持窗口随时间漂移——设计裕量需要预留额外的时序 guardband（保护带，通常 5-10%）以补偿老化退化

- 先进工艺中（<7nm），偏置温度不稳定性（BTI）和热载流子注入（HCI）等老化效应导致 D-FF 的建立/保持窗口随时间漂移——设计裕量需预留 5-10% 的额外时序 guardband 补偿老化退化
