...
- [[rtl-design/concepts/Sequential Logic_时序逻辑|时序逻辑]] — FSM 的状态存储在 D 触发器中，always_ff 描述状态寄存器的时钟和复位行为，时序约束决定了 FSM 的最大时钟频率。次态逻辑的延迟（从 current_state 到 next_state 的组合路径）是 FSM 时钟频率的最主要限制因素——独热编码通过减少次态逻辑层数来缓解这一约束
- [[rtl-design/concepts/Coding Style_RTL编码风格|RTL 编码风格]] — 三进程 FSM 模板、enum 命名约定、次态逻辑 case 结构的规范化要求和 lint 规则。FSM 编码风格是编码规范中最严格的一类——非法状态恢复策略（default 分支）和输出完整赋值是综合和 Lint 检查的重点
- [[rtl-design/concepts/Pipeline Design_RTL流水线|流水线设计]] — FSM 控制流水线的 stall/flush 和数据冒险处理，流水线控制器本质是 FSM + Datapath 的典型实例。流水线控制器的 FSM 状态（IDLE、ACTIVE、STALLed、FLUSHing）严格对应流水线寄存器使能和清零的控制信号
- [[rtl-design/concepts/SystemVerilog_系统Verilog|SystemVerilog]] — enum 类型的状态定义、always_ff/always_comb 的意图显式编码、unique case 的互斥性声明。SystemVerilog 的 `enum` 在仿真波形中显示状态名称而非比特值，是调试效率和团队沟通的质变
