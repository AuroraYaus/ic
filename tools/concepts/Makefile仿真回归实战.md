...
本篇属于 Part 9，目标是：用 Makefile 组织仿真、seed、testlist、日志、覆盖率和失败重跑。 读完后，读者应该能把一个最小例子复制到临时目录中运行，观察 GNU Make 4.3 如何解析规则、比较时间戳并执行配方。
...
- 需要知道文件修改时间会影响增量构建判断。
- 如果正在顺序学习，建议先读 [[tools/concepts/Makefile小型工程实战|前一篇]]，再读 [[tools/concepts/Makefile综合流程实战|后一篇]]。

## 最小可运行例子

在空目录中创建 `Makefile`，复制下面的内容，然后运行 `make --trace`。示例目标是 `logs/smoke.log`，它足够小，便于观察每一步行为。

```makefile
# 默认目标：用户只输入 make 时，Make 会选择第一个普通目标
all: logs/smoke.log          # all 是目标；冒号右边的文件是前置条件

# 真实文件目标：当 logs/smoke.log 不存在或依赖更新时执行配方
logs/smoke.log: input.txt    # input.txt 比目标新时，目标需要重建
...
clean:                 # 清理构建产物
	@rm -rf logs input.txt  # 删除示例产物，方便重新实验
```
...
- `all: logs/smoke.log` 表示 `all` 依赖 `logs/smoke.log`；`all` 放在最前面，因此成为默认目标。
- `logs/smoke.log: input.txt` 表示真实文件目标依赖输入文件；当输入比目标新时，目标需要重建。
- 配方行前面的 TAB 是 Make 语法要求，不是排版习惯；用空格替代会导致解析错误。
- `$@` 是自动变量，代表当前目标名；在这个例子中会展开为 `logs/smoke.log`。
...
- [[tools/concepts/Makefile小型工程实战|前一篇]]：提供本篇需要的前置背景或相邻概念。
- [[tools/concepts/Makefile综合流程实战|后一篇]]：把本篇概念推进到下一层工程用法。
- [[tools/工具与脚本|工具与脚本]]：本系列所在的工具领域内容地图。

## 小练习

1. 把 `logs/smoke.log` 改成另一个文件名，观察 `$@` 的输出如何变化。
