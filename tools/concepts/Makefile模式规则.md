...
tags:
  ...
  - makefile
  - asic
source_spec: "GNU Make Manual: Pattern Rules, Static Pattern Rules, Directory Search"
queries: 1
---
...
本篇属于 Part 5，目标是：理解 %、stem、静态模式规则和搜索路径，写出可扩展的编译规则。 读完后，读者应该能把一个最小例子复制到临时目录中运行，观察 GNU Make 4.3 如何解析规则、比较时间戳并执行配方。
...
- 需要知道文件修改时间会影响增量构建判断。
- 如果正在顺序学习，建议先读 [[tools/concepts/Makefile宏与元编程|前一篇]]，再读 [[tools/concepts/Makefile隐含规则|后一篇]]。
...
- [[tools/concepts/Makefile宏与元编程|前一篇]]：提供本篇需要的前置背景或相邻概念。
- [[tools/concepts/Makefile隐含规则|后一篇]]：把本篇概念推进到下一层工程用法。
