...
时序检查的核心是建立时间（Setup Time）和保持时间（Hold Time）验证。建立时间要求数据信号必须在时钟有效沿（Capture Edge）之前的一段最小时间内稳定——这段时间就是寄存器的建立时间要求（T_setup）。如果数据到达太晚，寄存器可能采样到错误的"旧"值或进入亚稳态。保持时间要求数据信号在时钟有效沿之后的一段最小时间内必须保持稳定——这段时间就是寄存器的保持时间要求（T_hold）。如果数据变化太早（在上一个值被可靠锁存之前就翻转），寄存器同样可能采样失败。

STA 工具（如 Synopsys PrimeTime、Cadence Tempus）通过计算以下不等式来验证每条路径：

- **建立时间检查**：`T_launch + T_clk2q_max + T_comb_max + T_setup < T_capture + T_period - T_uncertainty_setup`。即从发射时钟沿经发射寄存器时钟到输出延迟（T_clk2q）、组合逻辑最大延迟（T_comb_max）、再加建立时间要求，必须小于捕获时钟沿的时间。
- **保持时间检查**：`T_launch + T_clk2q_min + T_comb_min > T_capture + T_hold + T_uncertainty_hold`。即从发射时钟沿经最小延迟的数据路径、必须晚于捕获寄存器的保持时间要求。
...
时钟延迟（Clock Latency）分为源延迟（Source Latency，芯片外部的时钟网络延迟）和网络延迟（Network Latency，芯片内部的时钟树延迟）。在综合阶段，时钟树尚未实现，网络延迟以模型的估算值代入；在 CTS 之后，可反标（Back-annotate）真实的时钟树延迟。

时钟不确定性（Clock Uncertainty）是 STA 中人为施加的余量，用于覆盖抖动（Jitter，时钟周期的时间变化）、偏斜（Skew，同一时钟到达不同寄存器的时间差）和额外设计余量。建立时间检查使用较大的不确定性（加大约束），保持时间检查使用较小的不确定性。
...
- 四种时序路径类型——in2reg、reg2reg、reg2out、in2out——每一种的约束来源和优化方法都不同
...
- OCV（统一降额）→ AOCV（路径深度相关降额）→ POCV/LVF（统计性降额）是时序签核精度持续提升的演进路线
- 建立违例修复手段：降频、减少逻辑级数、up-size 门、调整寄存器位置；保持违例修复手段：插入 buffer/delay cell、增加线长
- 时序报告（Timing Report）是 STA 分析的核心输出，理解到达时间、要求时间、slack 和数据路径的每级延迟是时序分析的基本功
- 伪路径（False Path）和多周期路径（Multi-Cycle Path）如果不正确标注，会导致 STA 工具对不存在的违例进行无效优化，浪费 runtime 和面积
...
- [[asic-flow/concepts/signoff|签核（Signoff）]] — STA 是时序签核的核心，从 OCV→AOCV→POCV→LVF 的演进是签核精度持续提升的体现
- [[cross-domain/timing-closure|跨领域 — 时序收敛]] — 从综合到签核的跨阶段时序优化方法论，STA 是度量时序是否收敛的唯一标准
